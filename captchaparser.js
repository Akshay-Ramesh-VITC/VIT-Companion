...
const solveImageToText = async (imgEl) => {
  if (!imgEl) {
    return "";
  }
  ...
  try {
    await solve(imgEl, tempInput);
    const modelText = String(tempInput.value || "").trim();
    if (modelText) {
      return modelText;
    }
  } catch (_error) {
    // Fall through to bitmap captcha parser.
  }

  try {
    const imageSource = imgEl.tagName === "IMG" ? imgEl.src : "";
    const imageData = await uri_to_img_data(imageSource);
    let arr = [];
    let newArr = [];
    for (let i = 0; i < imageData.data.length; i += 4) {
      const gval =
    ...
    while (arr.length) {
      newArr.push(arr.splice(0, 180));
    }
    return String(captcha_parse(newArr) || "").trim();
  } catch (_error) {
    return "";
  }
};

window.VITCM_CAPTCHA = {
  solveImageToText
};
...
if (window.VITCM_CAPTCHA_AUTORUN === true) {
