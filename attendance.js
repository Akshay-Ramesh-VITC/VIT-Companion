...
const isAttendancePage = () => {
	return Boolean(
		document.getElementById("AttendanceDetailDataTable") ||
			document.querySelector(".table-responsive table")
	);
};

const isAttendanceApplied = () => Boolean(document.getElementById("attendance"));
...
chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
	if (request?.message === "check_attendance_view_applied") {
		sendResponse({
			onAttendancePage: isAttendancePage(),
			applied: isAttendanceApplied()
		});
		return;
	}
	...
	if (request?.message === "view_attendance") {
		if (!isAttendancePage()) {
			sendResponse({ ok: false, reason: "not-attendance-page" });
			return;
		}
		...
		try {
			...
			sendResponse({ ok: true, applied: isAttendanceApplied() });
		} catch (_error) {
			sendResponse({ ok: false, reason: "apply-failed" });
		}
		return true;
	}
});
